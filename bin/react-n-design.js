...
async function getAvailableComponents() {
  ...
  return [
    'Accordion', 'AIChat', 'AIThinking', 'Alert', 'Avatar', 'Badge', 
    'Breadcrumbs', 'Button', 'Calendar', 'Card', 'Carousel', 'Charts',
    'Checkbox', 'CodeBlock', 'Collapsible', 'ColorPicker', 'ComboBox',
    'CommandPalette', 'DataGrid', 'DatePicker', 'Divider', 'Drawer',
    'FileUpload', 'Form', 'Grid', 'Icon', 'Input', 'Markdown', 'Menu',
    'Modal', 'MultiSelect', 'Popover', 'ProgressBar', 'PromptInput',
    'RadioGroup', 'ScrollArea', 'Select', 'Skeleton', 'SkipToContent',
    'Slider', 'Stack', 'Stepper', 'SuggestionChips', 'Switch', 'Table',
    'Tabs', 'Tag', 'Toast', 'Toggle', 'Tooltip', 'Tree', 'Typography',
    'VirtualList', 'VisuallyHidden'
  ];
}
...
function fixImports(content, componentName) {
...
async function installSingleComponent(componentName) {
  ...
  try {
    ...
    if (!files) {
      ...
      process.stdout.write(`\r  ${colors.yellow}⚠${colors.reset} Component not found locally, checking GitHub...`);
...
function detectDependencies(files) {
  ...
  for (const content of Object.values(files)) {
    ...
    if (content.includes("from 'styled-components'") || content.includes('from "styled-components"')) {
...
async function main() {
  ...
  if (command === 'add') {
    ...
    log(`\n${bold('Summary:')}`);
    log(`  ${colors.green}${successful.length} installed${colors.reset}${failed.length > 0 ? `, ${colors.red}${failed.length} failed${colors.reset}` : ''}`);
