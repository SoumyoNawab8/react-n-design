...
import { FaCheck, FaChevronDown, FaCircle } from '../../icons';
...
export const Menu = memo(
  ({
    ...
    const enabledIndices = useMemo(
      () => items.map((item, i) => (isItem(item) && !item.disabled ? i : -1)).filter((i) => i >= 0),
      [items]
    );
    ...
    const handleKeyDown = useCallback(
      ...
      [isOpen, items, enabledIndices, activeIndex, close, handleItemClick]
    );
        ...
        {isOpen && (
          <>
            ...
            <MenuDropdown
              ...
              animate={
                isMobile && mobileDrawer ? { opacity: 1, y: 0 } : { opacity: 1, y: 0, scale: 1 }
              }
              exit={
                isMobile && mobileDrawer
                  ? { opacity: 0, y: '100%' }
                ...
                if (isItem(item)) {
                  ...
                  return (
                    ...
                    >
                      ...
                      {item.badge && (
                        <MenuItemBadge $color={item.badgeColor}>{item.badge}</MenuItemBadge>
                      )}
                    </MenuItem>
                  );
                }
